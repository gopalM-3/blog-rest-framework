...
class BlogUserAccountManager(BaseUserManager):
    ...
    def create_superuser(self, email, user_name, full_name, password, **other_fields):
        ...
        if other_fields.get('is_staff') is not True:
            raise ValueError(
                'Superuser must have to is_staff=True.')
        if other_fields.get('is_superuser') is not True:
            raise ValueError(
                'Superuser must have to is_superuser=True.')
...
class BlogUser(AbstractBaseUser, PermissionsMixin):
    ...
    objects = BlogUserAccountManager()
